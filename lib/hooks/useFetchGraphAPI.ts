...
export async function useFetchGraphAPI(query = '', { variables } = {}) {
  ...
  if(!config.wpGraphQlBaseURL) throw new Error('wpGraphQlBaseURL is missing from NextWP global config -- this is required to use useFetchGraphAPI().')
  // if(!config.wpAuthRefreshToken) throw 'wpAuthRefreshToken is missing from NextWP global config -- this is required to use useFetchGraphAPI().'
